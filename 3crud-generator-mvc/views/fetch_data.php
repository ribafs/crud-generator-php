...
if($nr > 0){
    ...
    while($row = $results->fetch(PDO::FETCH_ASSOC)) {
        echo "<tr>" . $controller->rowFields($row);
            ...
            <td><a onclick="return confirm('Realmente excluir o cliente <?=$name?> ?')" href="views/delete.php?id=<?=$id?>&table=<?=$table?>"><i class="glyphicon glyphicon-remove-circle" title="Delete"></a></td></tr>
    <?php
    print "
        </tr>";
    }
