...
if($conn->sgbd == 'mysql'){
    $results = $crud->pdo->prepare("SELECT * FROM $crud->table ORDER BY id DESC LIMIT $start, $conn->regsPerPage");
}else if($conn->sgbd == 'pgsql'){
    $results = $crud->pdo->prepare("SELECT * FROM $crud->table ORDER BY id DESC LIMIT $conn->regsPerPage OFFSET $start");
}
